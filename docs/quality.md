...
| gitleaks 相当 | 秘匿情報検出 | `just secrets` ターゲットとして追加予定 (T1.12 残部分) |
| pymarkdown | Markdown lint | `uv run pymarkdown -c .pymarkdown.json scan --recurse --respect-gitignore .` |

### 1.2 mypy 厳格度

型ヒントは最初から厳格に管理する。`pyproject.toml` 推奨設定:
...
# 秘匿情報検出 (gitleaks 相当) — T1.12 残部分で just secrets として追加予定
```
...
レポジトリにコミットされる予定のファイルに API キー風の文字列がないかを検出。`just secrets` (T1.12 残部分で将来追加) と一部重複するが、pytest からも呼べるようにしておく。
...
コード変更
  ↓
just check  ← 層1 + 層3  (secrets は just secrets として別途追加予定)
  ↓ green
新タスクの受入条件のテスト追加 ← 層2
  ↓ pytest green
...
| 4 | ambient agent 学習が進んでいる | 週次、Sprint 2 以降 | ユーザーが観察、レポートは自動 |
