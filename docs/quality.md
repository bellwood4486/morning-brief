...
secrets は pre-commit hook (gitleaks) がコミット前に自動スキャンする。allowlist は `.gitleaks.toml` で管理。
...
レポジトリにコミットされる予定のファイルに API キー風の文字列がないかを検出。pre-commit hook (gitleaks) と検出意図は重複するが、`pytest tests/architecture/` だけ走らせた場合の独立な検証経路として保持。
