...
- [ ] 毎日 06:30 JST に `#newsletter-digest` にダイジェストが自動投稿される
...
### Modal 実行で `invalid_grant: Token has been expired or revoked.`

Gmail OAuth refresh token が失効した状態。OAuth consent screen が **Testing (External)**
のままだと Google 仕様で refresh token は **7 日で自動失効**する。再発行手順:

```bash
uv run python scripts/bootstrap_oauth.py
```

ブラウザで再認可すると `gmail_oauth.json` が再生成され、Modal Secret `gmail-oauth` が
自動で上書きされる。完了後 `just dry-run` で疎通を確認する。

> **恒久対策**: GCP Console で OAuth app を **Publish (In production)** に切り替えると
> 7 日失効が消えるが、`gmail.modify` は機微スコープのため Google verification の対象になり
> 得る。自分専用 app であれば未検証のままでも継続利用可能。
