...
```bash
modal secret create slack-bot-token SLACK_BOT_TOKEN=xoxb-xxxx...
```

### 成功確認

```bash
modal secret list  # slack-bot-token が表示される
```
...
```bash
modal secret create gmail-oauth GMAIL_OAUTH_JSON=<スクリプトが出力した値>
```
...
```bash
modal secret list  # gmail-oauth が表示される
...
```bash
modal secret list  # gemini-api-key が表示される
```
...
## 10. Modal アカウント初期化と Volume 作成

### 10.1 Modal CLI のセットアップ

`modal` は `pyproject.toml` に依存として含まれているため `just sync` でインストール済み。
初回は認証トークンを発行する。

```bash
modal token new  # ブラウザが開いてログイン・トークン発行
```
...
```bash
modal volume create morning-brief-hermes
```

### 成功確認

```bash
modal volume list  # morning-brief-hermes が表示される
modal secret list  # slack-bot-token / gmail-oauth / gemini-api-key の 3 つが表示される
```
...
```bash
just dry-run  # = modal run modal_app.py::digest_job --dry-run
```
...
```bash
modal app list  # morning-brief が deployed 表示される
```
...
```bash
just run  # = modal run modal_app.py::digest_job
```
...
```bash
modal volume list  # 一覧確認
modal volume delete <不要な Volume 名>  # 余分なものを削除
```
