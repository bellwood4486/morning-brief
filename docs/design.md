...
- `update_if_ready()`: feedback.jsonl に一定量のフィードバックが蓄積されたら Gemini (PydanticAI 経由) に `seeds/USER.md` の差分を生成させ、GitHub PR を作成する (ADR-014)
- PR 化の実行主体は Modal Function 内の gh CLI (T2.5 で設計詳細化)
- 更新サイクルは日次 Cron 起点。即時性は不要 (ADR-006 と整合)
- Modal Volume `/root/.brief/` には `state/last_digest.json` と `feedback.jsonl` のみ。`seeds/USER.md` / `seeds/MEMORY.md` はリポジトリで Git 管理 (ADR-014)

### 2.8 `seeds/`

- リポジトリで管理される seeds ファイル群。`user_initial.md` 以外は人間マージと Gemini 提案で徐々に育つ
- `USER.md`: ユーザープロファイル (好むトピック / 嫌うトピック / 英語レベル等)。Gemini が diff 提案 → 人間マージのループで更新される (ADR-014)
...
```python
gmail_client.mark_processed(emails)
user_md_updater.update_if_ready()  # feedback が蓄積していれば Gemini で diff 生成 → GitHub PR 化
```
...
**決定**: 平日 06:30 JST 起動は Modal の `modal.Cron` で行う。外部エージェントや別プロセスの cron は使わない。
...
**決定**: Hermes Agent を完全廃止し、Python コードがオーケストレーションを担う「境界B」構成に移行する。Gemini は「要約」と「USER.md / MEMORY.md 更新差分の生成」の 2 サブルーチンとして呼ぶ。
