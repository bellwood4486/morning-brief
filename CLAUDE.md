...
8. **暗黙の決定をしない**: 設計に書かれていない判断が必要になったら、実装する前に Yoshiharu に確認する。
...
- **言語**: Python 3.11+。Yoshiharu は Go 歴 20 年・Python 学習中なので、型ヒントは厳格に書く (`mypy --strict` 相当)。
- **パッケージマネージャ**: uv。
...
- 上記に書かれていない判断は、暗黙にせず Yoshiharu に確認する
